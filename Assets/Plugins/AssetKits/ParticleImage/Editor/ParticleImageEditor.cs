...
namespace AssetKits.ParticleImage.Editor
{
    ...
    public class ParticleImageEditor : UnityEditor.Editor
    {
        ...
        private void OnSceneGUI()
        {
            ...
            switch (_particle.shape)
            {
                ...
                case EmitterShape.Rectangle:
                    ...
                    Handles.DrawWireCube(Vector3.zero,
                        new Vector3(_particle.rectWidth - _particle.emitterThickness,
                            _particle.rectHeight - _particle.emitterThickness));

                    break;
                case EmitterShape.Line:
                    Handles.DrawLine(new Vector3(-_particle.lineLength / 2, 0, 0),
                        new Vector3(_particle.lineLength / 2, 0, 0));
                    break;

                case EmitterShape.Directional:
                    Handles.DrawWireArc(Vector3.zero, Vector3.forward, PointInCircle(_angle.floatValue / 2).normalized,
                        _angle.floatValue, 100);
        ...
        private void UndoRedoPerformed()
        {
            ...
            if (EditorWindow.focusedWindow.titleContent.text == "Curve Editor") EditorWindow.focusedWindow.Close();
        }
        ...
        private void DrawSceneWindow(SceneView sceneView)
        {
            Handles.BeginGUI();
            ...
            if (GUILayout.Button("Stop", new GUIStyle("ButtonRight"))) _particle.Stop(true);

            GUILayout.EndHorizontal();
            EditorGUILayout.LabelField("Playback Time",
                _particle.main ? _particle.main.time.ToString("F") : _particle.time.ToString("F"));
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(_playMode, new GUIContent("Play*", "Shared between the particles group"),
                false);
            ...
            if (_particle.moduleEmitterFoldout)
            {
                ...
                switch (_particle.shape)
                {
                    ...
                    case EmitterShape.Rectangle:
                        ...
                        if (!_surface.boolValue)
                            EditorGUILayout.PropertyField(_edge, false);
                        _edge.floatValue = Mathf.Clamp(_edge.floatValue, 0f,
                            _particle.rectTransform.sizeDelta.x < _particle.rectTransform.sizeDelta.y
                                ? _particle.rectWidth
                                : _particle.rectHeight);
                        EditorGUILayout.PropertyField(_spreadType, new GUIContent("Spread"), false);
                        if (_particle.spreadType == SpreadType.Uniform)
                            EditorGUILayout.PropertyField(_spreadLoop, new GUIContent("Loop"), false);
                        break;
            ...
            if (_particle.moduleParticleFoldout)
            {
                ...
                if (_sheetModule.FindPropertyRelative("enabled").boolValue)
                {
                    ...
                    switch (_particle.textureSheetType)
                    {
                        case SheetType.Lifetime:
                            ...
                            var c = _particle.textureSheetFrameOverTime;
                            switch (_particle.textureSheetFrameOverTime.mode)
                            {
                                case ParticleSystemCurveMode.Curve:
                                    c.curveMultiplier = Mathf.Clamp(_particle.textureSheetFrameOverTime.curveMultiplier,
                                        0, _particle.textureTile.x * _particle.textureTile.y);
                                    break;
                                ...
                                case ParticleSystemCurveMode.TwoConstants:
                                    c.constantMax = Mathf.Clamp(_particle.textureSheetFrameOverTime.constantMax, 0,
                                        _particle.textureTile.x * _particle.textureTile.y);
                                    break;
                                case ParticleSystemCurveMode.Constant:
                                    c.constant = Mathf.Clamp(_particle.textureSheetFrameOverTime.constant, 0,
                                        _particle.textureTile.x * _particle.textureTile.y);
                                    break;
                            }
                    ...
                    var sf = _particle.textureSheetStartFrame;
                    switch (_particle.textureSheetStartFrame.mode)
                    {
                        case ParticleSystemCurveMode.Curve:
                            sf.curveMultiplier = Mathf.Clamp(_particle.textureSheetStartFrame.curveMultiplier, 0,
                                _particle.textureTile.x * _particle.textureTile.y);
                            break;
                        case ParticleSystemCurveMode.TwoCurves:
                            sf.curveMultiplier = Mathf.Clamp(_particle.textureSheetStartFrame.curveMultiplier, 0,
                            ...
                            break;
                        case ParticleSystemCurveMode.TwoConstants:
                            sf.constantMax = Mathf.Clamp(_particle.textureSheetStartFrame.constantMax, 0,
                                _particle.textureTile.x * _particle.textureTile.y);
                            break;
                        case ParticleSystemCurveMode.Constant:
                            sf.constant = Mathf.Clamp(_particle.textureSheetStartFrame.constant, 0,
                                _particle.textureTile.x * _particle.textureTile.y);
                            break;
                    }
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    else
                    {
                        ...
                        for (var i = 0; i < ch.Length; i++) DestroyImmediate(ch[i].gameObject);
                    }
                }
                ...
                if (_trailModule.FindPropertyRelative("enabled").boolValue)
                {
                    EditorGUILayout.HelpBox(
                        "Please note that trails are in the beta stage and are not optimized for production use. Therefore, it is recommended that you apply for as few particles as possible.",
                        MessageType.Warning);
                    ...
                    EditorGUILayout.PropertyField(_trailColorOverLifetime, new GUIContent("Color over Lifetime"),
                        false);
            ...
            if (_particle.moduleMovementFoldout)
            {
                ...
                if (_velocityModule.FindPropertyRelative("enabled").boolValue)
                {
                    ...
                    EditorGUILayout.PropertyField(_velocityOverLifetime, new GUIContent("Velocity Over Lifetime"),
                        false);
                }
        ...
        private void DrawHorizontalLine(int height = 1)
        {
            ...
            var rect = GUILayoutUtility.GetRect(10, height, GUILayout.ExpandWidth(true));
            rect.height = height;
            rect.xMin = 18;
            rect.xMax = EditorGUIUtility.currentViewWidth - 18;

            var lineColor = new Color(0.15f, 0.15f, 0.15f, 1);
        ...
        private bool Foldout(string title, bool display, Texture icon)
        {
            ...
            {
                EditorStyles.foldout.Draw(new Rect(rect.width - 4f, rect.y + 4f, 13f, 13f), false, false, display,
                    false);

...
namespace AssetKits.ParticleImage.Editor
{
    ...
    public class SeparateMinMaxCurveDrawer : PropertyDrawer
    {
        ...
        private GUIContent separateAxesContent
        {
            get
            {
                if (_separateAxesContent == null)
                    _separateAxesContent = new GUIContent(EditorGUIUtility.IconContent("d_AvatarPivot").image,
                        "Separate Axes");
        ...
        {
            var separable = property.FindPropertyRelative("separable").boolValue;
            ...
            var main = new Rect(separable ? position.x + 21 : position.x, position.y,
                separable ? position.width - 21 : position.width, 20);
            var x = new Rect(separable ? position.x + 21 : position.x, position.y,
                separable ? position.width - 21 : position.width, 20);
            var y = new Rect(separable ? position.x + 21 : position.x, position.y + 22,
                separable ? position.width - 21 : position.width, 20);
            var z = new Rect(separable ? position.x + 21 : position.x, position.y + 22 * 2,
                separable ? position.width - 21 : position.width, 20);

...
namespace Febucci.UI.Core
{
    ...
    public static class TextUtilities
    {
        ...
        {
            for (byte j = 0; j < vec.Length; j++) vec[j] = Vector3.LerpUnclamped(vec[j], target, pct);
        }
        ...
        {
            var middlePos = vec.GetMiddlePos();
            for (byte j = 0; j < vec.Length; j++) vec[j] = vec[j].RotateAround(middlePos, angle);
        }

        public static void RotateChar(this Vector3[] vec, float angle, Vector3 pivot)
        {
            for (byte j = 0; j < vec.Length; j++) vec[j] = vec[j].RotateAround(pivot, angle);
        }
        ...
        {
            for (byte j = 0; j < col.Length; j++) col[j] = Color32.LerpUnclamped(col[j], target, pct);
        }
        ...
        internal static void Initialize()
        {
            ...
            var randomDirections = new List<Vector3>();
            ...
            int randomIndex;
            for (var i = 0; i < fakeRandoms.Length; i++)
            {
        ...
        public static Vector3 RotateAround(this Vector3 vec, Vector2 center, float rotDegrees)
        {
            ...
            var tempX = vec.x - center.x;
            var tempY = vec.y - center.y;

            var rotatedX = tempX * Mathf.Cos(rotDegrees) - tempY * Mathf.Sin(rotDegrees);
            var rotatedY = tempX * Mathf.Sin(rotDegrees) + tempY * Mathf.Cos(rotDegrees);

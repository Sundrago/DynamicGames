...
namespace Lofelt.NiceVibrations
{
    ...
    /// 
    /// HapticSource provides a per-clip
    /// <c>MonoBehaviour</c>
    /// API for the functionality
    /// in HapticController, while HapticReceiver provides a
    /// <c>MonoBehaviour</c>
    /// API
    /// for the global functionality in HapticController.
    /// <c>HapticSourceInspector</c>
    /// provides a custom editor for HapticSource for the
    /// Inspector.
    ...
    public class HapticSource : MonoBehaviour
    {
        ...
        /// <c>HapticSourceInspector</c>
        /// . 0 is the highest priority and 256
        ...
        [SerializeField] private float _frequencyShift;
        ...
        public float level
        {
            ...
            set
            {
                ...
                if (IsLoaded()) HapticController.clipLevel = _level;
            }
        }
        ...
        public float frequencyShift
        {
            ...
            set
            {
                ...
                if (IsLoaded()) HapticController.clipFrequencyShift = _frequencyShift;
            }
        }
        ...
        {
            if (HapticController.IsPlaying() && IsLoaded()) Stop();
        }
        ...
        /// it will jump to that position if \ref loop is
        /// <c>false</c>
        /// . If \ref loop
        /// is
        /// <c>true</c>
        ...
        /// 
        /// It will loop playback in case \ref loop is
        /// <c>true</c>
        /// .
        public void Play()
        {
            if (CanPlay())
            {
                ...
                if (seekTime != 0.0f && !loop) HapticController.Seek(seekTime);
        ...
        public void Stop()
        {
            if (IsLoaded()) HapticController.Stop();
        }

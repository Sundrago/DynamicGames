...
#endif
{
    ...
    private const string IRONSOURCE_MANIFEST_PATH =
 "IronSource/Plugins/Android/IronSource.androidlib/AndroidManifest.xml";
...
#endif
    {
        if (File.Exists(IronSourceMediatedNetworkSettings.MEDIATION_SETTINGS_ASSET_PATH) || File.Exists(IronSourceMediationSettings.IRONSOURCE_SETTINGS_ASSET_PATH))
        {
            ...
            {
                string appId =
 IronSourceMediatedNetworkSettingsInspector.IronSourceMediatedNetworkSettings.AdmobAndroidAppId;
            ...
            {
                IEnumerable<XElement> permissons =
 elemManifest.Descendants().Where(elem => elem.Name.LocalName.Equals(MANIFEST_PERMISSION));

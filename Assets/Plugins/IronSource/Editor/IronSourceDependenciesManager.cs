...
public class IronSourceDependenciesManager : EditorWindow
{
    ...
    private readonly SortedSet<ProviderInfo> providersSet = new(new ProviderInfoComparor());
    ...
    private void OnGUI()
    {
        ...
        if (!string.IsNullOrEmpty(messageData))
        {
            using (new EditorGUILayout.VerticalScope("box", GUILayout.ExpandHeight(true)))
            {
                ...
                {
                    EditorGUILayout.SelectableLabel(messageData, EditorStyles.textField, GUILayout.ExpandWidth(true),
    ...
    private static string GetVersionFromXML(string fileName)
    {
        ...
        var version = IronSourceDependenciesManagerConstants.NONE;
        try
        {
        ...
        if (unityVersion != null) return unityVersion.InnerText;
    ...
    {
        var unityWebRequestLinkJson =
            UnityWebRequest.Get(IronSourceDependenciesManagerConstants.IRONSOURCE_SDK_XML_LINKS);
        var unityWebRequesInfoJson = UnityWebRequest.Get(IronSourceDependenciesManagerConstants.IRONSOURCE_SDK_INFO);
        var webRequestLinks = unityWebRequestLinkJson.SendWebRequest();
        var webRequestSDKInfo = unityWebRequesInfoJson.SendWebRequest();

        while (!webRequestLinks.isDone || !webRequestSDKInfo.isDone) yield return new WaitForSeconds(0.1f);

#if UNITY_2020_1_OR_NEWER
        if (unityWebRequestLinkJson.result != UnityWebRequest.Result.ProtocolError &&
            unityWebRequesInfoJson.result != UnityWebRequest.Result.ProtocolError)
#else
...
#endif
        {
            var linksJson = unityWebRequestLinkJson.downloadHandler.text;
            var SDKInfoJson = unityWebRequesInfoJson.downloadHandler.text;
            providersSet.Clear();

            var linksDictionary = new Dictionary<string, object>();
            var SDKInfoDictionary = new Dictionary<string, object>();
            try
            {
            ...
            {
                Debug.Log("Error getting response " + e);
            }

            if (SDKInfoDictionary != null && SDKInfoDictionary.Count != 0 && linksDictionary != null &&
                linksDictionary.Count != 0)
            {
                ...
                ironSourceProviderInfo.currentUnityVersion =
                    GetVersionFromXML(IronSourceDependenciesManagerConstants.IRONSOURCE_XML);

                SDKInfoDictionary.TryGetValue(IronSourceDependenciesManagerConstants.LATEST_SDK_VERSION,
                    out providersJson);
                ...
                requiredVersion =
                    ironSourceProviderInfo.currentUnityVersion == IronSourceDependenciesManagerConstants.NONE
                        ? ironSourceProviderInfo.latestUnityVersion
                        : ironSourceProviderInfo.currentUnityVersion;
                ...
                if (SDKInfoDictionary.TryGetValue(requiredVersion, out providersJson))
                    if (providersJson != null)
                        foreach (var item in providersJson as Dictionary<string, object>)
                        {
                            ...
                            object providerXML;
                            var lowerCaseItem = item.Key.ToLower(new CultureInfo("en"));

                            linksDictionary.TryGetValue(lowerCaseItem, out providerXML);

                            if (info.SetProviderDataProperties(item.Key, item.Value as Dictionary<string, object>,
                                    providerXML as Dictionary<string, object>))
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (item.Key.ToLower()
                                            .Contains(IronSourceDependenciesManagerConstants.UNITYADS))
                                        {
                                            if (File.Exists(
                                                    IronSourceDependenciesManagerConstants.IRONSOURCE_DOWNLOAD_DIR +
                                                    IronSourceDependenciesManagerConstants.UNITYADS_XML))
                                            {
                                                unityAdsProviderInfo.currentUnityVersion =
                                                    GetVersionFromXML(IronSourceDependenciesManagerConstants
                                                        .UNITYADS_XML);
                ...
                if (ironSourceProviderInfo.currentStatues == ProviderInfo.Status.INSTALLED ||
                    ironSourceProviderInfo.currentStatues == ProviderInfo.Status.NONE)
                {
                    if (SDKInfoDictionary.TryGetValue(IronSourceDependenciesManagerConstants.UPDATE_MSG,
                            out providersJson)) messageData = providersJson.ToString();
                }
                else
                {
                    if (SDKInfoDictionary.TryGetValue(IronSourceDependenciesManagerConstants.LATEST_MSG,
                            out providersJson)) messageData = providersJson.ToString();
                }
            }
        }
    ...
    {
        var win = GetWindowWithRect<IronSourceDependenciesManager>(
            new Rect(0, 0, IronSourceDependenciesManagerConstants.WIDTH, IronSourceDependenciesManagerConstants.HEIGHT),
            true);
        win.titleContent = new GUIContent(IronSourceDependenciesManagerConstants.INTERGRATION_MANAGER_TITLE);
        win.Focus();
    }

    private void DrawProviderItem(ProviderInfo providerData)
    {
        if (!providerData.Equals(default(ProviderInfo)))
            using (new EditorGUILayout.HorizontalScope(GUILayout.ExpandWidth(false)))
            {
                GUI.enabled = true;
                var isNew = providerData.isNewProvider;
                var isNewAddition = isNew ? IronSourceDependenciesManagerConstants.NEW_NETWORK : string.Empty;

                var tooltipText = IronSourceDependenciesManagerConstants.TOOLTIP_LATEST_VERSION + " \n " +
                                  providerData.providerName + " " +
                                  IronSourceDependenciesManagerConstants.TOOLTIP_ADAPTER_VERSION + " " +
                                  providerData.latestUnityVersion;
                if (providerData.sdkVersionDic.TryGetValue(IronSourceDependenciesManagerConstants.ANDROID,
                        out var androidVersion))
                    tooltipText = tooltipText + "\n " + IronSourceDependenciesManagerConstants.TOOLTIP_ANDROID_SDK +
                                  " " + androidVersion;
                if (providerData.sdkVersionDic.TryGetValue(IronSourceDependenciesManagerConstants.IOS,
                        out var iosVersion))
                    tooltipText = tooltipText + "\n " + IronSourceDependenciesManagerConstants.TOOLTIP_IOS_SDK + " " +
                                  iosVersion;

                EditorGUILayout.LabelField(providerData.displayProviderName + isNewAddition,
                    isNew ? boldTextStyle : textStyle);
                ...
                var btn = GUILayout.Button(gUIContent, buttonWidth);
    ...
    private void DrawSDKHeader()
    {
        using (new EditorGUILayout.HorizontalScope(GUILayout.ExpandWidth(false)))
        {
            EditorGUILayout.LabelField(IronSourceDependenciesManagerConstants.LABEL_CURRENT_SDK,
            ...
            GUILayout.Space(85);
            EditorGUILayout.LabelField(IronSourceDependenciesManagerConstants.LABEL_LATEST_SDK,
                new GUIStyle(EditorStyles.label)
                {
                    fontStyle = FontStyle.Bold,
                    fontSize = 13,
    ...
    {
        var fileNameIndex = downloadFileUrl.LastIndexOf("/") + 1;
        var downloadFileName = downloadFileUrl.Substring(fileNameIndex);
        var fileDownloading = string.Format("Downloading {0}", downloadFileName);
        var genericFileName = Regex.Replace(downloadFileName, @"_v+(\d\.\d\.\d\.\d|\d\.\d\.\d)", "");
        var path = Path.Combine(IronSourceDependenciesManagerConstants.IRONSOURCE_DOWNLOAD_DIR, genericFileName);
        ...
        if (!downloadWebClient.isHttpError && !downloadWebClient.isNetworkError)
            ...
            while (!downloadWebClient.isDone)
            {
                ...
                {
                    if (downloadWebClient.error != null) Debug.LogError(downloadWebClient.error);
                    CancelDownload();
                    isCancelled = true;
                }
            }
        ...
        if (genericFileName.EndsWith(".unitypackage") && !isCancelled)
        {
            AssetDatabase.ImportPackage(
                Path.Combine(IronSourceDependenciesManagerConstants.IRONSOURCE_DOWNLOAD_DIR, genericFileName), true);
        }
        else
        {
            // in case the download was cancelled, delete the file
            if (isCancelled &&
                File.Exists(IronSourceDependenciesManagerConstants.IRONSOURCE_DOWNLOAD_DIR + genericFileName))
    ...
    private static bool isNewerVersion(string current, string latest)
    {
        ...
        try
        {
            var currentVersion = Array.ConvertAll(current.Split('.'), int.Parse);
            var remoteVersion = Array.ConvertAll(latest.Split('.'), int.Parse);
            var remoteBuild = 0;
            var curBuild = 0;
    ...
    public class ProviderInfo
    {
        ...
        public bool SetProviderDataProperties(string provider, Dictionary<string, object> providerData,
            Dictionary<string, object> providerXML)
        {
            ...
            if (providerData.TryGetValue(IronSourceDependenciesManagerConstants.PROVIDER_UNITY_ADAPTER_VERSION,
                    out obj))
            {
                ...
                downloadURL = downloadURL.Replace(IronSourceDependenciesManagerConstants.UNITY_ADAPTER_MACRO,
                    latestUnityVersion);
            }

    ...
    public IronSourceImpressionData(string json)
    {
        if (json != null)
            try
            {
                ...
                var jsonDic = Json.Deserialize(json) as Dictionary<string, object>;
                ...
                if (jsonDic.TryGetValue(IronSourceConstants.IMPRESSION_DATA_KEY_REVENUE, out obj) && obj != null &&
                    double.TryParse(string.Format(invCulture, "{0}", obj), NumberStyles.Any, invCulture,
                        out parsedDouble)) revenue = parsedDouble;

                if (jsonDic.TryGetValue(IronSourceConstants.IMPRESSION_DATA_KEY_LIFETIME_REVENUE, out obj) &&
                    obj != null &&
                    double.TryParse(string.Format(invCulture, "{0}", obj), NumberStyles.Any, invCulture,
                        out parsedDouble)) lifetimeRevenue = parsedDouble;

                if (jsonDic.TryGetValue(IronSourceConstants.IMPRESSION_DATA_KEY_CONVERSION_VALUE, out obj) &&
                    obj != null &&
                    int.TryParse(string.Format(invCulture, "{0}", obj), NumberStyles.Any, invCulture, out parsedInt))
                    conversionValue = parsedInt;
            }
            catch (Exception ex)
            {
                Debug.Log("error parsing impression " + ex);
            }
    }

...
public class iOSAgent : IronSourceIAgent
{
    ...
    private static extern void LPPSetWaterfallConfiguration(IOSWaterfallConfiguration configurationParams,
        AdFormat adFormat);
    ...
    [DllImport("__Internal")]
    private static extern void CFLoadBanner(string description, int width, int height, int position,
        string placementName, bool isAdaptive);
    ...
    private delegate void ISUnityPauseGame(bool pause);
    ...
    public int? getConversionValue()
    {
        ...
        int parsedInt;
        if (int.TryParse(string.Format(invCulture, "{0}", CFGetConversionValue()), NumberStyles.Any, invCulture,
                out parsedInt)) return parsedInt;
    ...
    [MonoPInvokeCallback(typeof(ISUnityPauseGame))]
    ...
    public IronSourcePlacement getPlacementInfo(string placementName)
    {
        ...
        var spString = CFGetPlacementInfo(placementName);
        if (spString != null)
        {
            var spDic = Json.Deserialize(spString) as Dictionary<string, object>;
            var pName = spDic["placement_name"].ToString();
            var rewardName = spDic["reward_name"].ToString();
            var rewardAmount = Convert.ToInt32(spDic["reward_amount"].ToString());
    ...
    {
        CFLoadBanner(size.Description, size.Width, size.Height, (int)position, placementName, size.IsAdaptiveEnabled());
    }
    ...
    {
        var dict = segment.getSegmentAsDict();
    ...
    {
        var json = Json.Serialize(impressionData);

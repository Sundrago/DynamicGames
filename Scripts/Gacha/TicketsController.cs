...
public class TicketsController : MonoBehaviour
{
    ...
    private int CalculateBonus(int score, int midScore)
    {
        ...
        if (MyUtility.PlayerData.GetInt(DataKey.totalTicketCount) < 500 && score >= midScore / 2f) ticketCount += 1;
        if (MyUtility.PlayerData.GetInt(DataKey.totalTicketCount) < 200 && score >= midScore) ticketCount += 1;

...
namespace Games.Land
{
    ...
    public class VirtualJoystickController : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
    {
        ...
        private JoystickState CalcJoystickState(float degree, float distance)
        {
            ...
            else if ((degree > -60f) & (degree < 0)) return JoystickState.Left;
            else if ((degree > -120f) & (degree < -60f)) return JoystickState.Up;
            else if ((degree > -180f) & (degree < -120f)) return JoystickState.Right;
            else return JoystickState.Idle;
        }
    }
}
...
namespace DynamicGames.MainPage
{
    ...
    public class BlockDragHandler : MonoBehaviour, IDraggable
    {
        ...
        {
            mainPage.DeactivateAllBlocksExcept(gameObject);
        ...
        private void OnMouseDrag()
        {
            ...
            var targetPosition = Vector3.Lerp(transform.position, currentMousePosition, 0.3f);
            targetPosition.z = initialPos.z;
            transform.position = targetPosition;

            var delta = Vector2.Distance(dragDeltaPosition, Input.mousePosition);
        ...
        private bool IsBlockClicked()
        {
            ...
            mainPage.GotoSelectedGame(blockType, gameObject);
            return false;
        }
        ...
        private List<PetObject> GetActivePets()
        {
            var activePets = new List<PetObject>();
            foreach (var petData in PetManager.Instance.GetActivePetConfigs())
            {
                var isActive = petData.obj.activeSelf;
                var isOnCurrentCorner =
                    petData.obj.GetComponent<PetSurfaceMovement2D>().currentLocation.obj == gameObject;
                if (isActive && isOnCurrentCorner)
                {
                    var petObject = petData.obj.GetComponent<PetObject>();
        ...
        private void ShakePets(List<PetObject> pets)
        {
            var threshold = pets.Count < 3 ? 1 : 2.5f / pets.Count;

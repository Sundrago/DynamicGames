...
{
    /// <summary>
    ///     Responsible for playing sound effects based on collision size and playing fail sound effects.
    /// </summary>
    public class SFXManager : MonoBehaviour
    {
        ...
        private void PlayAudioClip(AudioClip[] clips, float volumeMin, float volumeMax)
        {
            ...
            audioSource.PlayOneShot(clips[rnd],
                PlayerData.GetFloat(DataKey.settings_sfx) * Random.Range(volumeMin, volumeMax));
        }
        ...
        {
            var rnd = Random.Range(0, fail.Length);
            audioSource.pitch = Random.Range(0.8f, 1.2f);
            audioSource.PlayOneShot(fail[rnd],
                PlayerData.GetFloat(DataKey.settings_sfx) * Random.Range(0.6f, 1f));
        }
    }
}
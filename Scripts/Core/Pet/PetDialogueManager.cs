...
namespace Core.Pet
{
    ...
    {
        [SerializeField]
        public Dictionary<PetType, PetDialogueData> petDialogueDatas = new Dictionary<PetType, PetDialogueData>();
        ...
        public string GetIdleText(PetType _type)
        {
            ...
            if (petDialogueDatas[_type].idle0 != "")
                strings.Add(MyUtility.Localize.GetLocalizedPetDialogue(petDialogueDatas[_type].idle0));
            if (petDialogueDatas[_type].idle1 != "")
                strings.Add(MyUtility.Localize.GetLocalizedPetDialogue(petDialogueDatas[_type].idle1));
            if (petDialogueDatas[_type].idle2 != "")
                strings.Add(MyUtility.Localize.GetLocalizedPetDialogue(petDialogueDatas[_type].idle2));
            ...
            switch (_scoreType)
            {
                case PetScoreType.score_newBest:
                    if (string.IsNullOrEmpty(petDialogueDatas[_type].score_newBest))
                        return MyUtility.Localize.GetLocalizedPetDialogue(petDialogueDatas[PetType.Default]
                            .score_newBest);
